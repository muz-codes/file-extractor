...
func (p *Policy) Sanitize(s string) []string {
	_, lines := p.sanitizeWithBuff(strings.NewReader(s))
	var scrap []string
	for i := range lines {
...
func (p *Policy) sanitizeWithBuff(r io.Reader) (*bytes.Buffer, []string) {
	var buff bytes.Buffer
	var lines []string
	if err := p.sanitize(r, &buff, &lines); err != nil {
...
func (p *Policy) sanitize(r io.Reader, w io.Writer, lines *[]string) error {
	...
	for {
		...
		case html.TextToken:
			...
			if !skipElementContent {
				...
				case `script`:
					// not encouraged, but if a policy allows JavaScript
					if p.allowUnsafe {
				...
				case "style":
					// not encouraged, but if a policy allows CSS styles
					if p.allowUnsafe {
						if _, err := buff.WriteString(token.Data); err != nil {
							return err
						}
				...
				default:
					...
					trimData := strings.Trim(token.String(), " ")
					if strings.Trim(token.String(), " ") == "" {
						continue
					}
					if _, err := buff.WriteString(trimData); err != nil {
						return err
					}
					*lines = append(*lines, trimData)
				}
			}
